...
export default function App() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("toonstyle_saved_styles");
    if (stored) {
      try {
  ...
  const generateCartoon = async () => {
    ...
    if (!prompt && !subjectImage) {
      setError("Please provide a description or a subject image.");
      return;
    }
    ...
    try {
      ...
      const fullPrompt = subjectImage 
        ? `Create a caricature/cartoon of the person in the last image, strictly adhering to ${styleRef}. ${prompt}`
        : `Create a cartoon of "${prompt}" strictly following ${styleRef}.`;
        ...
        <div className="p-8 border-r border-black flex flex-col gap-10">
          ...
          <section className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold uppercase tracking-widest flex items-center gap-2">
                <User className="w-4 h-4" /> Subject & Prompt
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div 
                ...
                className={cn(
                  "relative aspect-square border-2 border-dashed border-black rounded-xl overflow-hidden cursor-pointer transition-all hover:bg-[#F0F0F0] group",
                  subjectImage ? "border-solid" : "bg-[#F9F9F9]"
                )}
              >
                <input 
                  type="file" 
                ...
                {subjectImage ? (
                  <>
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <p className="text-white font-bold text-xs">Change Subject</p>
                    </div>
                    <button 
                      onClick={(e) => { e.stopPropagation(); setSubjectImage(null); }}
                      className="absolute top-2 right-2 p-1 bg-white border border-black rounded-full hover:bg-[#00FF00]"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4 text-center">
                    ...
                    <p className="text-xs font-bold">Person Image<br/><span className="font-normal opacity-60">(Optional)</span></p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                <textarea
                  placeholder="Describe the character or scene... (e.g. 'A cool cat wearing sunglasses')"
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  className="flex-1 p-4 border-2 border-black rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-[#00FF00] resize-none font-medium text-sm"
                />
              </div>
            </div>
          </section>
          ...
          <div className="mt-auto pt-6">
            ...
            <button
              ...
              disabled={isGenerating || styleImages.length === 0 || (!prompt && !subjectImage)}
              className={cn(
